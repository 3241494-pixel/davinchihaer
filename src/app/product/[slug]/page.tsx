...
export async function generateMetadata({
  ...
  if (!product) return {};
  return {
    title: product.seo.title.ru,
    description: product.seo.description.ru,
  };
}
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <h1 className="font-heading text-3xl text-ink-strong">{product.title.ru}</h1>
              <p className="text-base text-ink-muted">{product.shortDescription.ru}</p>
            </div>
            <VariantSelector />
            <VariantPriceActions />
            <TrustBadges />
